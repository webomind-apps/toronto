...
@section('main')
    ...
    <form action="{{ route('admin.city.update',$city) }}" enctype="multipart/form-data" method="post">
        @csrf
        @method("put")
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control" name="country_id" id="country_id">
                            ...
                            @foreach ($countries as $key => $country)
                                ...
                                {{isSelected($country->id, $city->country_id)}}
                                value="{{ $country->id }}">
                                    {{ $country->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control" name="province_id" id="province_id">
                            ...
                            @foreach ($provinces as $key => $province)
                                <option
                                {{isSelected($province->id, $city->province_id)}}
                                 value="{{ $province->id }}">
                                    {{ $province->name }}
                                </option>
                            @endforeach
