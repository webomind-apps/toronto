...
class CreateBusinessTempsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('business_temps', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->string('name');
            $table->string('transaction_id')->unique();
            $table->string('email')->unique();
            $table->string('phone')->unique();
            $table->foreignId('user_temp_id')->nullable()->constrained('user_temps', 'id')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('business_temps');
    }
}
